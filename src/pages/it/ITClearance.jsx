...
import {
  ...
  ChevronDown, ChevronUp, Users, Search, SlidersHorizontal,
  X, CheckCheck, Clock, TrendingUp
} from 'lucide-react'
...
export default function ITClearance() {
  ...
  const bulkSubmitAll = async (row) => {
    ...
    if (successCount > 0) toast.success(`${successCount} item${successCount > 1 ? 's' : ''} submitted successfully`)
    await reloadItems(row.id)
      ...
      <div className={s.pg}>
        <div className={s.emptyWrap}>
          ...
          <div className={s.emptyText}>
            <h3>Access Denied</h3>
            <p>You don't have permission to view IT clearance requests. Contact your administrator if you believe this is a mistake.</p>
      ...
      <div className={s.pg}>
        <div className={s.hdr}>
          ...
          <div className={s.hdrContent}>
            <div className={s.hdrEyebrow}><span className={s.hdrEyebrowDot} /><span>IT Department</span></div>
            <div className={s.hdrMain}>
              <div><h1 className={s.hdrTitle}>IT Clearance</h1><p className={s.hdrSub}>Manage and verify employee asset returns</p></div>
            </div>
          </div>
        </div>
        <div className={s.emptyWrap}>
    ...
    <div className={s.pg}>
      ...
      <div className={s.statsRow}>
        ...
        <div className={s.statCard}>
          ...
          <div className={s.statInfo}>
            <span className={s.statVal}>{list.filter(r => ['today','urgent'].includes(getLwdStatus(r.proposedLastWorkingDate).type)).length}</span>
      ...
      <div className={s.accordion}>
        {filteredList.map(row => {
          ...
          return (
            <div key={row.id} className={`${s.panel} ${isExpanded ? s.panelOpen : ''} ${lwdStatus.type === 'overdue' && pendingExists ? s.panelOverdue : ''}`}>
                ...
                <div className={s.panelBadges}>
                  <span className={`${s.lwdBadge} ${lwdBadgeClass}`}>
                    {lwdStatus.type === 'overdue'
                      ? `Overdue ${lwdStatus.daysAgo}d`
                      : lwdStatus.type === 'today'
                      ? 'Due Today'
                      : lwdStatus.type === 'urgent'
                      ? `${lwdStatus.daysLeft}d left`
                      : `LWD ${lwdFormatted}`}
                  </span>
                ...
                <div className={s.panelBody}>
                  ...
                  {isLoadingRow ? (
                    <div className={s.loadingItems}>
                      ...
                      Loading IT clearance items…
                    </div>
                  ) : items.length === 0 ? (
                    <div className={s.noItems}><Info size={14} /> No IT clearance items found.</div>
                  ) : (
                        ...
                        return (
                          <div key={item.id} className={`${s.itemCard} ${st.isCleared ? s.itemCleared : s.itemFlagged}`}>
                            ...
                            <div className={s.decRow}>
                              <button type="button" className={`${s.decBtn} ${st.isCleared ? s.decCleared : ''}`} onClick={() => setField(item.id, 'isCleared', true)}>
                                <CheckCircle size={12} /> Cleared
                              </button>
                              <button type="button" className={`${s.decBtn} ${!st.isCleared ? s.decFlagged : ''}`} onClick={() => setField(item.id, 'isCleared', false)}>
                            ...
                            {st.isCleared && (
                              <div className={s.extraField}>
                                ...
                                <input
                                  type="date" max={lwdStr} value={st.returnedDate}
                                  onChange={e => setField(item.id, 'returnedDate', e.target.value)}
                                  style={{ borderColor: errs.returnedDate ? '#DC2626' : undefined }}
                                />
                                {errs.returnedDate && <span className={s.fieldErr}><AlertTriangle size={11} />{errs.returnedDate}</span>}
                              </div>
                            )}
                            ...
                            {!st.isCleared && (
                              <div className={s.extraField}>
                                ...
                                />
                                {errs.pendingDueAmount && <span className={s.fieldErr}><AlertTriangle size={11} />{errs.pendingDueAmount}</span>}
                              </div>
                            )}
                            ...
                            <div className={s.extraField}>
                              ...
                              {errs.remarks && <span className={s.fieldErr}><AlertTriangle size={11} />{errs.remarks}</span>}
                            </div>

                            <Button variant="primary" size="sm" loading={submittingId === item.id} onClick={() => submitItem(item, row)}>
